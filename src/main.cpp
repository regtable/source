...
unsigned int nStakeMinAge = 8 * 60 * 60; // testing 1 hr ...8 hours
...
const string strMessageMagic = "OculorCoin Signed Message:\n";
...
{
    int64_t nSubsidy = 1 * COIN + nFees;
    
    if(nHeight <= PREMINE_BLOCK)
    {
        nSubsidy =  750000000 * COIN; //  PREMINE 3,000,000,0000 4 blocks of 750000000
    }
    else if(nHeight < FAIR_LAUNCH_BLOCK)
    {
        nSubsidy = 1 * COIN + nFees;
    }
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees)
{
    int64_t percentages = 0 * CENT;
     ...
     int64_t nSubsidy = nCoinAge * percentages * 33 / (365 * 33 + 8); //integer equivalent of nCoinAge * nRewardCoinYear
...
int nTargetSpacing = 150; //2.5 min
...
#ifdef ENABLE_WALLET
// oculorcoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("OculorCoin-loadblk");
...

{
    ...
    int64_t ret = blockValue * 10.0; //10% for masternodes
    ...
    int64_t ret = blockValue * 0.0; //0% for masternodes
    ...
    int64_t ret = blockValue * 0.10; //10% for masternodes

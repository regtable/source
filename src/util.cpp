...
int nAnonymizeOculorCoinAmount = 500;
...
#else
    const char* pszModule = "OculorCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OculorCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OculorCoin
    // Mac: ~/Library/Application Support/OculorCoin
    // Unix: ~/.OculorCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OculorCoin";
#else
    fs::path pathRet;
...
void createConf()       //Automatic oculorcoin.conf generation
{
    srand(time(NULL));
    ...
    const char* nodes =  "\nrpcport=48844"
                         ...
                         "\nserver=1"
                         "\nlistenonion=0"
                         "\naddnode=dnsseed1.oculorcoin.net"
                         "\naddnode=dnsseed2.oculorcoin.net";
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "oculorcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "oculorcoind.pid"));

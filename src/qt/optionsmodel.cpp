...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeOculorCoinAmount"))
        settings.setValue("nAnonymizeOculorCoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeOculorCoinAmount = settings.value("nAnonymizeOculorCoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeOculorCoinAmount"))
        SoftSetArg("-anonymizeOculorCoinamount", settings.value("nAnonymizeOculorCoinAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeOculorCoinAmount:
            nAnonymizeOculorCoinAmount = value.toInt();
            settings.setValue("nAnonymizeOculorCoinAmount", nAnonymizeOculorCoinAmount);
            emit anonymizeOculorCoinAmountChanged(nAnonymizeOculorCoinAmount);
            break;
        default:
            break;
        }
